...
function NotificationItem({ notification }) {
  ...
  const acceptRequest = () => {
    fetch(
      `${process.env.REACT_APP_API_URL}/users/request-accepted/${notification.from._id}/${user._id}/${notification._id}`,
      {
        headers: {
  ...
  const declineRequest = () => {
    fetch(
      `${process.env.REACT_APP_API_URL}/users/request-declined/${user._id}/${notification._id}`,
      {
        headers: {
          "Content-Type": "application/json; charset=utf-8",
          "x-auth-token": localStorage.getItem("token"),
    ...
    <div className="notifications-item-container">
      <div className="notification-half">
        <div className="notification-top">
          ...
          <p className="notification-p bold">
            {notification.dateCreated + " " + notification.timeCreated}
          </p>
        </div>
