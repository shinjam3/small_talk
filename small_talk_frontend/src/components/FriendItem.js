...
function FriendItem({friend, history}) {
	...
	const createGroup = () => {
		fetch(`${process.env.REACT_APP_API_URL}/groups/create-group-with-friend/${user._id}/${friend._id}`, {
			headers: {
	...
	return (
		<div>
			<div className='connect-user'>
				...
				<button className='connect-user-button friend' onClick={createGroup}>Message</button>
				<button className='connect-user-button friend' onClick={showWarning}>Remove Friend</button>
			</div>
