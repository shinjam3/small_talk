vimport React, { useState, useContext } from "react";
...
function ProfileNav({ history, page }) {
  ...
  return (
    <div className="profile-nav-container">
      <div className={navExpandClass} onClick={handleExpand}>
        &#9660;
      </div>
      <div className={notificationsClass} onClick={handleNotificationsClick}>
        Notifications
      </div>
      <div className={editClass} onClick={handleEditClick}>
        Edit Profile
      </div>
      <div className={settingsClass} onClick={handleSettingsClick}>
        Settings
      </div>
      <div className={logoutClass} onClick={handleLogout}>
        Log Out
      </div>
    </div>
  );
