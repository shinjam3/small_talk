...
function Register({history}) {
	...
	const userValidation = () => {
		...
		let tempArray = [], joiArray = [], termsArray = [];
		...
		if (registered) {
			return (
				<div className='login-container registered'>
					<h3>Thank you for registering. Click <span style={{cursor:'pointer'}} onClick={() => history.push('/')}><u>here</u></span> to sign into your account.</h3>
				</div>
			);
		}
		else {
			return (
				<div className={loginContainerClass}>
					...
					<div className={validationErrorClass}>
						{errorMsgs.map( msg => <div key={(Math.random()*1000).toString()} style={{paddingTop:'15px', paddingBottom:'15px'}}>{msg}</div> )}
					</div>
					
					<form className='login-form' onSubmit={handleSubmit}>
						<h2 className='login-h2'>Register</h2>
						
						<input className='login-input' type='text' placeholder='First Name' name='firstName' onChange={handleChange} value={userInfo.firstName} />
						<input className='login-input' type='text' placeholder='Last Name' name='lastName' onChange={handleChange} value={userInfo.lastName} />
						<input className='login-input' type='text' placeholder='Email' name='email' onChange={handleChange} value={userInfo.email} />
						<input className='login-input' type='password' placeholder='Password' name='pass' onChange={handleChange} value={userInfo.pass} />
						<input className='login-input' type='password' placeholder='Confirm Password' onChange={handleConfirmPassChange} value={confirmPass} />
						
						<div className='login-accept-div'>
							<input className='login-accept-input' type='checkbox' onChange={handleCheck} /> 
							<p> I accept the <span onClick={showTerms}><u>Terms & Conditions</u></span>.</p>
						</div>
						
						<button className='st-button' type='submit'>Register</button>
					</form>
					
					<div className={termsClass}>
						<h2>Terms & Conditions</h2>
						<p>Your password will be encrypted in our database.</p>
						<p>However, we strongly recommend that you register with an email and password that you do not use for any other service.</p>
