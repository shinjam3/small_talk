...
function Connect({history}) {
  ...
  const filterSearch = () => {
		...
		for(let i=0; i<usersArray.length; i++) {
			if (usersArray[i].friends.some(friend => friend._id===user._id)) usersArray[i].isFriend = true;
		};
			...
			<div>
				...
				<div className='content-body'>
					<form className='connect-form' onSubmit={(e) => e.preventDefault()}>
						<input className='connect-input' type='text' onChange={handleChange} placeholder='Search for a user...' value={searchInput} />
					</form>
