	...
	return (
		<div className={groupSettingsContainer}>
			<div className='group-settings-item'>
				<div className='group-settings-close' onClick={closeSettings}><p>&#x2715;</p></div>
				<div className='group-settings-top'>
					<p className='group-settings-p' onClick={expandMembers}>Members</p>
				</div>
				<div className={membersClass}>
					<p>{groupMembers.map(member => member.firstName).join(', ')}</p>
					<p className='group-settings-shrink' onClick={closeMembers}>^</p>
				</div>
			</div>
			
			<div className='group-settings-item'>
				<p className='group-settings-p' onClick={expandLeaveGroup}>Leave Group</p>
