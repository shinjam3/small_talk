...
function ConnectItem({otherUser}) {
	...
	const handleClick = () => {		
		fetch(`${process.env.REACT_APP_API_URL}/users/send-friend-request/${otherUser._id}/${user._id}`, {
			headers: {
	...
	return (
		<div>
			<div className='connect-user'>
				...
				<button className='connect-user-button' disabled={buttonDisabled} onClick={handleClick}>{buttonText}</button>
			</div>
		</div>
	);
};
